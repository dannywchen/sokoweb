...
const LifeStory = () => {
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      const shouldReveal = scrollTop + clientHeight >= scrollHeight - 200;

      if (shouldReveal && revealIndex < sentences.length - 1) {
  ...
  return (
    <>
      ...
      <Container ref={containerRef}>
        ...
        {sentences.slice(0, revealIndex + 1).map((text, index) => (
          <SentenceContainer
            key={index}
            initial="hidden"
            animate="visible"
